...
const SUPABASE_KEY = (process.env.SUPABASE_KEY || process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || '').trim();
if (!SUPABASE_KEY) {
  console.error('FATAL: SUPABASE_KEY / SUPABASE_SERVICE_KEY / SUPABASE_SERVICE_ROLE_KEY not set in environment. Set one of these Supabase service-role variables in Railway before boot.');
  process.exit(1);
}
...
app.get('/api/status', (req, res) => res.json({ ok: true, version: BOOT_INFO.version, db: 'supabase', openaiConfigured: !!(process.env.CUSTOM_CRED_API_OPENAI_COM_TOKEN || process.env.OPENAI_API_KEY) }));
app.get('/api/', (req, res) => res.json({ name: 'Hearth & Page API', version: BOOT_INFO.version, db: 'supabase' }));
...
app.get('/api/cases/:id/deadlines', requireAuth, async (req, res) => {
  try {
    ...
    res.json(rows || []);
  } catch (e) { res.status(500).json({ error: 'Failed to fetch deadlines' }); }
});
...
app.post('/api/cases/:id/deadlines', requireAuth, async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ error: 'Failed to save deadline' }); }
});
...
app.delete('/api/cases/:id/deadlines/:dlId', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: 'Failed to delete deadline' }); }
});

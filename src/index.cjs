...
const SUPABASE_KEY = process.env.SUPABASE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9tdXdpY2RiZXVhdm9qcm5kZHdlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI2NTA2MTIsImV4cCI6MjA5ODIyNjYxMn0.5CLOTUWz7V94arLGX2NmcoDK2uHNW4IXnj4I7UZPYsk';
...
app.get('/api/health', (req, res) => res.json({ ok: true, time: new Date().toISOString(), db: 'supabase' }));
...
app.patch('/api/cases/:caseId', requireAuth, async (req, res) => {
  try {
    const c = await dbGet('cases', { id: `eq.${req.params.caseId}`, user_id: `eq.${req.user.id}` });
    if (!c) return res.status(404).json({ message: 'Case not found' });
    const { title, caseType, status } = req.body;
